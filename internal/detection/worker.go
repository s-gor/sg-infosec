...
func NewWorker(source RecordSource, processor EventProcessor, correlator *Correlator) *Worker {
	return NewWorkerWithRules(source, processor, correlator, nil)
}

func NewWorkerWithRules(source RecordSource, processor EventProcessor, correlator *Correlator, rules *RuleSet) *Worker {
	if correlator == nil {
...
func (worker *Worker) Run(ctx context.Context) error {
	if worker == nil || worker.source == nil || worker.processor == nil || worker.correlator == nil || worker.rules == nil {
...
func (worker *Worker) handle(ctx context.Context, record JournalRecord) {
	findings := mergeUniqueFindings(Parse(record), worker.rules.Parse(record))
