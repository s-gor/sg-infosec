...
func New(cfg config.Config, dependencies Dependencies) (*App, error) {
	...
	eventsHandler := events.NewHandler(events.NewProcessor(database, dependencies.Clock, cfg.Policies...), cfg.EventBodyLimit)
	knownSources := make([]string, 0, len(cfg.Sources))
	for _, source := range cfg.Sources {
		knownSources = append(knownSources, source.ID)
	}
	controlHandler := control.NewHandler(decisionService, database, dependencies.Clock, knownSources, cfg.EventBodyLimit)
	...
	application.events, err = unixhttp.New(unixhttp.Config{
		SocketPath: cfg.EventsSocket, Mode: 0660, ExpectedOwnerUID: dependencies.UID,
	}, eventsHandler, resolver)
	if err != nil {
		cleanup()
		return nil, err
	}
	application.control, err = unixhttp.New(unixhttp.Config{
		SocketPath: cfg.ControlSocket, Mode: 0660, ExpectedOwnerUID: dependencies.UID,
	}, mux, resolver)
	if err != nil {
		cleanup()
		return nil, err
