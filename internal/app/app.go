...
func New(cfg config.Config, dependencies Dependencies) (*App, error) {
	...
	eventsProcessor := events.NewProcessor(database, dependencies.Clock, policies...).WithDecisionNotifier(application.nftWorker)
	if detectorEnabled {
		if dependencies.DetectorSource == nil {
			dependencies.DetectorSource = journal.New(journal.DefaultConfig())
		}
		application.detector = detection.NewWorker(dependencies.DetectorSource, eventsProcessor, nil)
	}
